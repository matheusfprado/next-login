...
export default function Header({ userEmail = "", cryptos }: HeaderProps) {
  ...
  const cryptoDrops = useMemo(() => {
    ...
    return cryptos.filter((coin: Crypto) => coin.price_change_percentage_24h < -5);
  }, [cryptos]);
    ...
    <header className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm relative">
      ...
      <div className="flex items-center gap-6 relative">
        <div ref={dropdownRef} className="relative">
          <button
            className="relative p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors flex items-center justify-center"
            onClick={() => setDropdownOpen(!dropdownOpen)}
          >
            <BellIcon className="w-6 h-6 text-gray-700 dark:text-gray-300" />
            {cryptoDrops.length > 0 && (
              <span className="absolute top-1 right-1 inline-flex h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white dark:ring-gray-900 animate-pulse"></span>
            )}
          </button>
          {dropdownOpen && (
